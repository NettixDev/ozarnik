...
impl HistoryCell for TooltipHistoryCell {
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        vec![Line::from(format!("Подсказка: {}", self.tip))]
    }
}
...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            "  Чтобы начать, опишите задачу или попробуйте одну из команд:"
                .dim()
                .into(),
            Line::from(""),
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " — создать AGENTS.md с инструкциями для ОЗАРНИК".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
                " — показать настройки текущей сессии".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " — выбрать что ОЗАРНИК разрешено делать".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
                " — выбрать модель и уровень reasoning".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/review".into(),
        ...
        if requested_model != session.model.as_str() {
            let lines = vec![
                ...
                format!("использовано: {}", session.model).into(),
            ];
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ ОЗАРНИК (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("ОЗАРНИК").bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{})", self.version)).dim(),
        ];

        const CHANGE_MODEL_HINT_COMMAND: &str = "/model";
        const CHANGE_MODEL_HINT_EXPLANATION: &str = " — сменить";
        const DIR_LABEL: &str = "папка:";
        const PERMISSIONS_LABEL: &str = "права:";
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let mut lines = vec![
            Line::from(format!("ОЗАРНИК (v{})", self.version)),
            Line::from(format!(
                "модель: {}{}",
                self.model,
        ...
        if self.yolo_mode {
            lines.push(Line::from("права: YOLO режим"));
        }
        lines
    }
}
