...
use codex_model_provider_info::ONLYSQ_API_KEY_ENV_VAR;
...
/// SQAgent first-run check: if no API key is stored and none is in the environment, prompt the
/// user for an OnlySQ API key on stdin and save it to `auth.json` so subsequent runs are silent.
fn ensure_onlysq_api_key_or_prompt() -> std::io::Result<()> {
    // Already in environment? Skip.
    if std::env::var(ONLYSQ_API_KEY_ENV_VAR)
        .ok()
    ...
    let codex_home = find_codex_home()?;
    let store_mode = AuthCredentialsStoreMode::default();
    if let Ok(Some(auth)) = load_auth_dot_json(&codex_home, store_mode)
        && auth.openai_api_key.as_deref().is_some_and(|k| !k.trim().is_empty())
    {
        // Existing key in auth.json — re-export it so downstream code that reads the env var sees it.
        if let Some(k) = auth.openai_api_key.as_deref() {
            unsafe {
                std::env::set_var(ONLYSQ_API_KEY_ENV_VAR, k);
            }
        }
        return Ok(());
    }
    ...
    if !std::io::stdin().is_terminal() {
        return Err(std::io::Error::other(
            "Первый запуск ОЗАРНИК требует ключ OnlySQ API, но stdin не TTY. Запустите `OZARNIK login --api-key <КЛЮЧ>` или установите переменную ONLYSQ_API_KEY.",
        ));
    }

    eprintln!("Добро пожаловать в ОЗАРНИК. Ключ OnlySQ API не найден.");
    eprintln!("Получить ключ можно на https://onlysq.ru — вставьте его ниже.");
    eprint!("Ключ OnlySQ API: ");
    use std::io::BufRead;
    use std::io::Write;
    ...
    if api_key.is_empty() {
        return Err(std::io::Error::other(
            "Ключ не введён. Прерываю.",
        ));
    }
    login_with_api_key(&codex_home, &api_key, store_mode)?;
    unsafe {
        std::env::set_var(ONLYSQ_API_KEY_ENV_VAR, &api_key);
    }
    eprintln!("Сохранено. Продолжаю.");
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    ensure_onlysq_api_key_or_prompt()?;
