...
impl ChatWidget {
    ...
    pub(super) fn current_model_service_tier_commands(&self) -> Vec<ServiceTierCommand> {
        // ОЗАРНИК: service-tier slash commands (e.g. /fast) are OpenAI-specific and not
        // applicable to OnlySQ. Always return an empty list so they are neither suggested
        // in the popup nor accepted when typed manually.
        let _ = self.current_model();
        let _ = &self.model_catalog;
        Vec::new()
    }
