...
//!
//! This module owns the auth-step state machine (OnlySQ login/device-code/API
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with OnlySQ to use ОЗАРНИК as part of your paid plan".into(),
            ]),
            Line::from(vec![
                "  ".into(),
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  API key login is disabled by this workspace. Sign in with OnlySQ to continue."
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Signed in with your OnlySQ account".fg(Color::Green).into(),
            "".into(),
            "  Before you start:".into(),
            "".into(),
            "  Decide how much autonomy you want to grant ОЗАРНИК".into(),
            Line::from(vec![
                "  For more details see the ".into(),
                "\u{1b}]8;;https://developers.openai.com/codex/security\u{7}ОЗАРНИК docs\u{1b}]8;;\u{7}".underlined(),
            ])
            .dim(),
            "".into(),
            "  ОЗАРНИК can make mistakes".into(),
            "  Review the code it writes and commands it runs".dim().into(),
            "".into(),
            "  Powered by your OnlySQ account".into(),
            Line::from(vec![
    ...
    fn render_chatgpt_success(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Signed in with your OnlySQ account"
                .fg(Color::Green)
                .into(),
        ];
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  ОЗАРНИК will use usage-based billing with your API key.".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "Use your own OnlySQ API key for usage-based billing".bold(),
            ]),
            "".into(),
    ...
    /// Kicks off the OnlySQ auth flow and keeps the UI state consistent with the attempt.
    fn start_chatgpt_login(&mut self) {
        // If we're already authenticated with OnlySQ, don't start a new login –
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            .await,
            feedback: codex_feedback::ОЗАРНИКFeedback::new(),
            log_db: None,
            state_db: None,
