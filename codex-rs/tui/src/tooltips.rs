...
const APP_TOOLTIP: &str = "Попробуйте **ОЗАРНИК App**. Запустите 'sqagent app' или зайдите на https://onlysq.ru";
const FAST_TOOLTIP: &str =
    "*Новое* Используйте **/fast** чтобы включить самый быстрый inference с расширенным лимитом.";
const OTHER_TOOLTIP: &str = "*Новое* Работайте быстрее с **ОЗАРНИК App**. Запустите 'sqagent app' или зайдите на https://onlysq.ru";
const OTHER_TOOLTIP_NON_MAC: &str = "*Новое* Работайте быстрее с ОЗАРНИК.";
const FREE_GO_TOOLTIP: &str =
    "*Новое* Ограниченное время — ОЗАРНИК включён в ваш план бесплатно, давайте строить вместе.";
...
/// Pick a random tooltip to show to the user when starting ОЗАРНИК.
...
pub(crate) mod announcement {
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // ОЗАРНИК currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
    ...
    fn blocking_init_announcement_tip() -> Option<String> {
        // ОЗАРНИК: do not fetch a remote announcement tip. Return a hardcoded local tip.
        Some(
            String::from(
                "[[announcements]]\ncontent = \"Чтобы работать — запустите OZARNIK из командной строки в папке проекта. Чтобы ОЗАРНИК не спрашивал подтверждений при правках — выберите /permissions full.\"\n",
            ),
        )
    }
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for ОЗАРНИК TUI.
...
[[announcements]]
content = "Welcome to ОЗАРНИК! Check out the new onboarding flow."
