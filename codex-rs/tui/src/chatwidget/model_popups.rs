...
impl ChatWidget {
    ...
    pub(crate) fn open_model_popup(&mut self) {
        ...
        // ОЗАРНИК: try OnlySQ live catalog first; fall back to local catalog on failure.
        if let Some(presets) = crate::onlysq_models::fetch_onlysq_presets_blocking()
            && !presets.is_empty()
        {
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        ...
        let warning = format!(
            "Внимание: базовый URL OnlySQ переопределён на {base_url}. Выбор моделей может работать некорректно."
        );
